...
შერჩევითი სწავლის მექანიზმი (Gating Mechanism): TFT იყენებს "Gating" მექანიზმებს, რათა შერჩევითად დაუშვას შესაბამისი ინფორმაცია ქსელში, რაც ეხმარება მას ხმაურის შემცირებაში და მხოლოდ ყველაზე მნიშვნელოვანი მახასიათებლების ფოკუსირებაში. როდესაც ჩვენ ინიციალიზაციას ვუკეთებთ TFT კლასს და ვავარჯიშებთ მას, ეს მექანიზმი ავტომატურად მუშაობს ფონზე, რაც მოდელს ეხმარება ისწავლოს რომელი დროითი ნაბიჯები ან რომელი კოვარიატებია ყველაზე მნიშვნელოვანი პროგნოზირებისთვის.

ყურადღების მექანიზმი (Attention Mechanism): Transformer-ის მსგავსად, TFT იყენებს ყურადღების მექანიზმს, რათა შეაფასოს წარსული დროითი ნაბიჯების მნიშვნელობა მომავალი პროგნოზირებისთვის.
ეს ფუნქციონალობა ინტეგრირებულია self.model_ = TFT(...) ინიციალიზაციისას და აქტიურდება self.nf_.fit(df=df) ვარჯიშის პროცესში. ყურადღების მექანიზმი საშუალებას აძლევს მოდელს დაადგინოს, თუ Weekly_Sales-ის პროგნოზირებისას, წარსულში არსებული 'Temperature', 'CPI', 'IsHoliday', 'MarkDown' და სხვა მახასიათებლების რომელი მნიშვნელობებია ყველაზე გავლენიანი.
...
ჩვენ კოდში ვიყენებთ: DateFeatureCreator: ქმნის week, sin_13, cos_13, sin_23, cos_23 ფუნქციებს Date სვეტიდან. ესენი არის ცნობილი მომავალი მახასიათებლები, რადგან მათი მნიშვნელობები ნებისმიერი თარიღისთვის წინასწარ არის ცნობილი. მაღაზიის Type არის სტატიკური მახასიათებელი, numerical_cols = ['Temperature', 'Fuel_Price', 'CPI', 'Unemployment']  ესენი მომავალში უცნობებია, ასევე მარკდაუნებიც უცნობია.
...
დროითი უწყვეტობის უზრუნველყოფა: ეს არის მნიშვნელოვანი ახალი ნაბიჯი. ჩვენ შევქმენით ყველა შესაძლო (Store, Dept, Date) კომბინაცია თითოეული სერიის მინიმალურ და მაქსიმალურ თარიღებს შორის, freq='W-FRI' (კვირა, პარასკევი) სიხშირის გამოყენებით. შემდეგ შევაერთეთ ეს სრული თარიღის დიაპაზონები ორიგინალ მონაცემებთან. 
...
Feature engeneering:
MultiIndexKeeper: ეს ტრანსფორმატორი აყენებს DataFrame-ის ინდექსს ['Date', 'Store', 'Dept'] MultiIndex-ად. ეს აუცილებელია, რადგან NeuralForecast მუშაობს MultiIndex-იან DataFrame-ებთან. მნიშვნელოვანია, რომ drop=False არგუმენტი გამოიყენება, რათა ეს სვეტები მონაცემებში დარჩეს როგორც კოვარიატები, გარდა ინდექსისა.
...
sin_13, cos_13: სინუსური და კოსინუსური ტრანსფორმაციები week-ზე 13-კვირიანი (დაახლოებით კვარტალური) სეზონურობის დასაფიქსირებლად.

sin_23, cos_23: სინუსური და კოსინუსური ტრანსფორმაციები week-ზე 23-კვირიანი სეზონურობის დასაფიქსირებლად (შეიძლება იყოს დამატებითი ციკლები).

ესენი არის "ცნობილი მომავალი კოვარიატები" (Known Future Covariates), რადგან მათი გამოთვლა ნებისმიერი მომავალი თარიღისთვის წინასწარ არის შესაძლებელი.

TFTRegressor კლასი:
__init__: კონსტრუქტორი იღებს TFT-ის ძირითად პარამეტრებს, როგორიცაა input_chunk_length (რამდენი წარსული დროითი ნაბიჯი გამოიყენება პროგნოზისთვის - 52 კვირა) და output_chunk_length (რამდენი მომავალი ნაბიჯი უნდა იყოს პროგნოზირებული - 39 კვირა). ასევე მოცემულია epochs და batch_size.

fit(X, y): ეს მეთოდი ამზადებს მონაცემებს NeuralForecast-ის მოლოდინის შესაბამისად:

y (Weekly_Sales) ერთიანდება X-თან.

Date სვეტიდან იქმნება ds (timestamp) სვეტი.

unique_id სვეტის შექმნა: df["unique_id"] = df["Store"].astype(str) + "_" + df["Dept"].astype(str). ეს არის კრიტიკული ნაბიჯი მრავალსერიული პროგნოზირებისთვის. NeuralForecast ამ unique_id-ს იყენებს თითოეული ინდივიდუალური დროითი სერიის (Store-Dept კომბინაცია) იდენტიფიცირებისთვის.

ინფუთად მოდელისთვის: self.nf_.fit(df=df)-ს გადაეცემა df DataFrame, რომელიც შეიცავს:
...
ყველა სხვა სვეტი: (როგორიცაა Temperature, Fuel_Price, CPI, Unemployment, MarkDownX სვეტები, Type და IsHoliday One-Hot encoded ვერსიები, sin/cos დროითი ფიჩერები, Store, Dept) ავტომატურად განიხილება NeuralForecast-ის მიერ, როგორც მახასიათებლები. მოდელი სწავლობს ამ კოვარიატების გამოყენებას y-ის პროგნოზირებისთვის.

predict(X): ეს მეთოდი გამოიყენება ახალი პროგნოზების შესაქმნელად:
ის იღებს X (ვალიდაციის ნაკრები) და ამზადებს მას, როგორც მომავალ კოვარიატებს.

self.nf_.make_future_dataframe(self.trained_df_) გამოიყენება NeuralForecast-ის მიერ მოსალოდნელი მომავალი თარიღების დასაგენერირებლად თითოეული unique_id-ისთვის.

self.nf_.predict(df=self.trained_df_, futr_df=futr_df_complete) მეთოდი აკეთებს პროგნოზირებას. df პარამეტრი გადასცემს ვარჯიშის მონაცემებს (საჭიროა ისტორიული კონტექსტისთვის), ხოლო futr_df გადასცემს მომავალ კოვარიატებს, რისთვისაც პროგნოზი კეთდება.
